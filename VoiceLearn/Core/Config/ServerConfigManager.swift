// VoiceLearn - Server Configuration Manager
...
public struct ServerConfig: Codable, Identifiable, Sendable {
    ...
    /// URL for discovery endpoint (VoiceLearn gateway)
...
public enum ServerType: String, Codable, Sendable, CaseIterable {
    case voicelearnGateway = "voicelearn"  // Our unified gateway
    ...
    public var displayName: String {
        switch self {
        case .voicelearnGateway: return "VoiceLearn Gateway"
    ...
    public var defaultPort: Int {
        switch self {
        case .voicelearnGateway: return 11400
    ...
    public var supportsDiscovery: Bool {
        self == .voicelearnGateway
    }
}

// MARK: - Discovered Service

/// A service discovered from a VoiceLearn gateway
...
public actor ServerConfigManager {
    ...
    private let logger = Logger(label: "com.voicelearn.serverconfig")
    ...
    public func getHealthyLLMServers() -> [ServerConfig] {
        servers.values.filter { server in
            ...
            (server.serverType == .ollama ||
             ...
             server.serverType == .voicelearnGateway ||
    ...
    /// Discover VoiceLearn servers on the local network
    ...
    private func probeServer(host: String, port: Int) async -> ServerConfig? {
        ...
        // Try VoiceLearn gateway discovery first
        if let gatewayServer = await probeVoiceLearnGateway(host: host, port: port) {
    ...
    /// Probe for VoiceLearn gateway
    private func probeVoiceLearnGateway(host: String, port: Int) async -> ServerConfig? {
        ...
        do {
            ...
            if let json = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
               let isVoiceLearn = json["voicelearn_server"] as? Bool,
               isVoiceLearn {
    ...
    public func getBestLLMEndpoint() -> URL? {
        if let server = getHealthyLLMServers().first,
           let baseURL = server.baseURL {
            // For VoiceLearn gateway, use the unified endpoint
            if server.serverType == .voicelearnGateway {
